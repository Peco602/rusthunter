...
use crate::plugins::{os, Plugin};
use crate::utils::{output_json, print_info, print_success, print_warning};

pub fn list(plugins: &Vec<&dyn Plugin>) -> Result<(), String> {
    println!("{: <32} {: <50} Operating System", "Plugin", "Description");
    println!("{:=<100}", "=");
    for p in plugins {
        p.show();
    }
